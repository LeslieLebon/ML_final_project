...
best_knn_stats["cross validation",] <- data.frame( 
                           ...
                           Specificity=knn_cv_mat[["byClass"]] [["Specificity"]],
...
best_knn_stats["repeatedcv ",] <- data.frame(
                           ...
                           Specificity=knn_repcv_mat[["byClass"]] [["Specificity"]],
...
best_knn_stats["cv z",] <- data.frame(
...
print.data.frame(best_knn_stats["cv z",])
...
plotted_knn<-ggplot(best_knn_stats_melt,aes(x = statistics, y = percentage, fill = method)) + geom_bar(position = 'dodge', stat='identity')
grid.arrange(tableGrob(best_knn_stats[,-6]), plotted_knn, nrow = 2, ncol = 1)
```
```{r message=FALSE,warning=FALSE}
#manually chosen confusion matrix (k=9)
print(knn_table_9_z)
#Caret's cross validation confusion matrix (z-score scaled)
print(knn_z_table[["t"]])
```
...
svm_tan_stats <- data.frame(method="Hyperbolic tangent", 
...
svm_stats["Hyperbolic tangent",]=svm_tan_stats
...
plotted_svm<-ggplot(best_svm_stats_melt,aes(x = statistics, y = percentage, fill = method)) + geom_bar(position = 'dodge', stat='identity')

grid.arrange(tableGrob(svm_stats), plotted_svm, nrow = 2, ncol = 1)
```
...
svm_tan_norm_stats <- data.frame(method="tan", 
...
```
```{r svm normalized data print, message=FALSE, warning=FALSE}
#print statistics
...
plotted_svm_norm<-ggplot(best_svm_norm_stats_melt,aes(x = statistics, y = percentage, fill = method)) + geom_bar(position = 'dodge', stat='identity')

grid.arrange(tableGrob(svm_norm_stats), plotted_svm_norm, nrow = 2, ncol = 1)
...
We tried using the data that we normalized and compared the 2.
```{r svm scaled data linear, message=FALSE, warning=FALSE, results='hide'}
# linear
...
plotted_svm_z<-ggplot(best_svm_z_stats_melt,aes(x = statistics, y = percentage, fill = method)) + geom_bar(position = 'dodge', stat='identity')

grid.arrange(tableGrob(svm_z_stats), plotted_svm_z, nrow = 2, ncol = 1)
...
```{r RF tree_res, echo=FALSE, results='hide', message=FALSE}
set.seed(345)
# define the workflow
tree_wf <- workflow() %>%
  add_model(tune_spec) %>%
