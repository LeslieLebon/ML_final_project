...
grid.arrange(tableGrob(knn_seperate_cv_stats_z), tableGrob(best_knn_stats), nrow=2)
```
...
train_cconfMat_rep_cv_z <- confusionMatrix(predict(train_with_knn_repeatedcv_z, train_set_z[,-63]),data_train[,63])
...
```{r message=FALSE,warning=FALSE}
knitr:: kables( list(
  knitr::kable(knn_table_9_z, caption = "manually chosen"),
  knitr::kable(knn_z_table[["t"]], caption = "Caret's cross validation")))
...
```{r svm_linear confusionMatrix, message=FALSE, warning=FALSE}
...
svm_stats["SVM linear",]<-svm_linear_stats
...
svm_stats["SVM RBF",]=svm_rbf_stats
...
svm_stats["SVM Polynomial",]=svm_poly_stats 
...
```
```{r SVM tan, message=FALSE, warning=FALSE, echo=FALSE}
# calculate performance
...
svm_stats["SVM Hyperbolic tangent",]=svm_tan_stats
```
...
```{r svm_best_stats_1, message=FALSE, warning=FALSE}
...
rownames(svm_best_stats)<-c("SVM linear norm")
...
svm_norm_stats["SVM rbf norm",]<-svm_rbf_norm_stats
...
svm_norm_stats["SVM poly norm",]<-svm_poly_norm_stats
...
svm_norm_stats["SVM tan norm",]<-svm_tan_norm_stats
```
```{r svm normalized data print, message=FALSE, warning=FALSE, echo=FALSE}
grid.arrange(tableGrob(svm_norm_stats),ncol=1,nrow=1)          
```
Add the best scores of the normalized
```{r svm_best_stats_2, message=FALSE, warning=FALSE}
svm_best_stats["SVM linear norm",]<-svm_norm_stats["SVM linear norm",]
```

### z-score scaled
We tried scaling with z-score as well
We tried using the data that we normalized and compared the 2.
...
svm_linear_z_stats <- data.frame(method="SVM linear z", 
                           Accuracy=svm_linear_z[["overall"]][["Accuracy"]],
                           AUC=pROC_svm_linear_z[["auc"]],
                           Sensitivity=svm_linear_z[["byClass"]] [["Sensitivity"]], 
                           Precision=svm_linear_z[["byClass"]] [["Precision"]], 
                           Specificity=svm_linear_z[["byClass"]] [["Specificity"]])
svm_z_stats<-svm_linear_z_stats
rownames(svm_best_stats)<-c("SVM linear z")
...
svm_rbf_z_stats <- data.frame(method="SVM rbf z", 
                           Accuracy=svm_rbf_z[["overall"]][["Accuracy"]],
                           AUC=pROC_svm_rbf_z[["auc"]],
                           Sensitivity=svm_rbf_z[["byClass"]] [["Sensitivity"]], 
                           Precision=svm_rbf_z[["byClass"]] [["Precision"]], 
                           Specificity=svm_rbf_z[["byClass"]] [["Specificity"]])
svm_z_stats["SVM rbf z",]<-svm_rbf_z_stats
...
svm_poly_z_stats <- data.frame(method="SVM poly z", 
                           Accuracy=svm_poly_z[["overall"]][["Accuracy"]],
                           AUC=pROC_svm_poly_z[["auc"]],
                           Sensitivity=svm_poly_z[["byClass"]] [["Sensitivity"]], 
                           Precision=svm_poly_z[["byClass"]] [["Precision"]], 
                           Specificity=svm_poly_z[["byClass"]] [["Specificity"]])
svm_z_stats["SVM poly norm",]<-svm_poly_z_stats
...
svm_tan_z_stats <- data.frame(method="SVM tan z", 
                           Accuracy=svm_tan_z[["overall"]][["Accuracy"]],
                           AUC=pROC_svm_tan_z[["auc"]],
                           Sensitivity=svm_tan_z[["byClass"]] [["Sensitivity"]], 
                           Precision=svm_tan_z[["byClass"]] [["Precision"]], 
                           Specificity=svm_tan_z[["byClass"]] [["Specificity"]])
svm_z_stats["SVM tan norm",]<-svm_tan_z_stats
```
```{r svm_z_stats, message=FALSE, warning=FALSE}
grid.arrange(tableGrob(svm_z_stats),ncol=1,nrow=1)          
```
Add the best scores of the normalized
```{r svm_best_stats_3, message=FALSE, warning=FALSE}
svm_best_stats["SVM linear z",]<-svm_z_stats["SVM linear z",]
#print best svm_best_stats
grid.arrange(tableGrob(svm_best_stats),ncol=1,nrow=1)          

```

## SVM conclusions

```{r svm conclusion and extra}
...
```{r rf_mod, echo=FALSE, results='hide', message=FALSE}
...
```{r rf_fit, echo=FALSE, results='hide', message=FALSE}
# train model
library(ranger)
set.seed(234)
rf_fit <- 
...
```{r rf_training_pred and accuracy calculation, echo=FALSE, results='hide', message=FALSE}
rf_training_pred <- 
...
```{r rf_testing_pred and accuracy calculation, echo=FALSE, results='hide', message=FALSE}
rf_testing_pred <- 
...
```{r RF workflow, echo=FALSE, results='hide', message=FALSE}
# create the folds
set.seed(345)
...
```{r RF Tuning hyperparameters, echo=FALSE, results='hide', message=FALSE}
...
```{r RF tree_grid, echo=FALSE, results='hide', message=FALSE}
...
```{r RF plot preformance, echo=FALSE, results='hide', message=FALSE}
# plot preformance
tree_res %>%
  collect_metrics() %>%
