...
#split_data_by_index<-data.frame(set=c("train","test"),glaucoma=c(train_div[1],testdiv[1]), normal=c(train_div[2],testdiv[2]))
...
#knitr::kable(split_data_by_index, caption = 'split_data by index')
```
...
```
```{r echo=FALSE}
#split_data<-data.frame(set=c("train","test"),glaucoma=c(train_div[1],testdiv[1]),normal=c(train_div[2],testdiv[2]))
print(paste0('The glaucoma/normal distribution was: train set = ',
       round(train_div[1],2),"/",round(testdiv[1],2), ', and test set = ',
       round(train_div[2],2),"/",round(testdiv[2],2)))
```
...
We saw that out of all the kernels, the standard Gaussian Radial basis function (RBF) gave the best result. We also saw that using the smaller dataset that included only the selected features did not yeild any better results. One last thing to note is that normalization yielded the same accuracy (86%), worse sensitivity (72% as opposed to 84%), but managed to get to 100% in both precision and specificity. 
...
t_svm_rbf <- tableGrob(svm_rbf[["table"]])
gtitle_svm_rbf <- textGrob("RBF confusion matrix")
gtable_svm_rbf <- gtable_add_rows(t_svm_rbf, heights = grobHeight(gtitle_svm_rbf)+ unit(5,"mm"), pos = 0)
gtable_svm_rbf <- gtable_add_grob(gtable_svm_rbf, gtitle_svm_rbf, 1, 1, 1, ncol(gtable_svm_rbf))

t_svm_rbf_norm <- tableGrob(svm_rbf_norm[["table"]])
gtitle_svm_rbf_norm <- textGrob("RBF norm confusion matrix")
gtable_svm_rbf_norm <- gtable_add_rows(t_svm_rbf_norm, heights = grobHeight(gtitle_svm_rbf_norm)+ unit(5,"mm"), pos = 0)
gtable_svm_rbf_norm <- gtable_add_grob(gtable_svm_rbf_norm, gtitle_svm_rbf_norm, 1, 1, 1, ncol(gtable_svm_rbf_norm))

#grid.arrange(gtable_svm_rbf ,gtable_svm_rbf_norm, ncol=2, nrow=1)
```
...
#grid.arrange( plotted_knn, tableGrob(best_knn_stats[,-7][,-5]),nrow = 2, ncol = 1)
...
using CV and repeated CV the best resukt was with K=17. We had manually chosen K=15 and K=21, and both were not as good as K=3.
...
#print.table(DT_boost_pred10_tbl[["table"]])
...
```
```{r message=FALSE, warning=FALSE, fig.height=3, fig.cap="Desition Tree", fig.width=8, echo=FALSE}
#print.table(dt_cost_pred_tbl[["table"]])
...
#knitr::kable(DT_pred_tbl[["table"]] , caption = 'confusion matrix of the best DT model') %>% kable_paper("striped", full_width = F) 
...
#print.data.frame(collect_metrics(rf_fit_rs)%>% select(.metric, mean, std_err))
#knitr::kable(collect_metrics(rf_fit_rs), caption = 'collect_metrics(rf_fit_rs)')
```
```{r message=FALSE, warning=FALSE, echo=FALSE,results='hide'}
...
#print.data.frame(best_rf_stats["10-fold CV",])
```
...
```{r RF plot preformance, message=FALSE, warning=FALSE, echo=FALSE, results='hide', fig.height=2}
tree_res %>%
  collect_metrics() %>%
...
```{r message=FALSE, warning=FALSE, fig.height=3, results='hide', fig.show='hide'}
...
best_stats["RF",]<-best_rf_stats["final_fit",]
...
Random forest classify patients with more sensitivity than decision tree. This result is not suprising by the fact that random forest build many decision trees and select the best.

# conclusions

```{r general conclusion and extra, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2}
knitr::kable(best_stats , caption = ' best RF stat results')%>%
  kable_paper("striped", full_width = F)  %>%
  column_spec(6, bold = T) %>%
  row_spec(1, bold = T, color = "black", background = "yellow")
...
ggroc(list(SVM=pROC_svm_rbf, KNN=pROC_knn_3_z_new, DF=pROC_DT, RF=pROC_RF_final))+labs(title = "all algotithms")
```

To summarize, our dataset is composed of 196 patients:98 sicks and 98 healthy.
There are 63 features corresponding to the blood vessels eye pressure which help to determine if a patient has glaucoma. Each feature has its degree of importance, some of them seem to be more important than others. Keeping only important features (based on threshold determine by varImp), we did not observe any improvement in the patient classification. Moreover, we observed that some features highly correlate. Unfortunately, after deleting highly correlated features, we did not observe a major improvement in the patient classification.Thereby, we decided to keep the 63 features. 
Before to test the different algorithms, we defined a weight for each variable allowing to evaluate the predictions: 3 for sensitivity, 2 for precision,  2 for accuracy and 1 for specificity, in order to classify results of the algorithm used. 

The last figure showing the sensitivity and the specificity help us to compare the different algorithm choosen in this project. We observe that KNN algorithm gives the best sensitivity score (0.92), so the highest rate of true positive cases, which is the most important variable.About the precision, (true predicted glaucoma/predicted glaucoma), type I error, KNN algorithm gives the better score (0.92) followed by DT algorithm (0.913). About the accurency (true prediction/ all prediction), proportion of correct prediction, KNN gives the best score again (0.92) follwed by DT algorithm (0.88). Fianlly, about the specificity, (true predicted healthy/healthy), KNN and DT algorithm give the same score (0.92). Combining all together, KNN algorithm gives the best predictions followed by decision tree algorithm. SVM and random forest give similar score of predictions, lower than KNN and decision tree.
