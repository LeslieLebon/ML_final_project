...
grid.arrange(default_pca_plot +
               labs(title = paste0(";hopkins=",round(hopkins(glaucoma[,1:62], m=nrow(default_pca$x)-1))),
...
fit_corr_pca  <- lm(Class ~ ., cbind(Class = glaucoma$Class, corr_pca$x[, 1:2]) %>% as.data.frame())
```
...
## SVM conclusions
We saw that out of all the kernels, the linear one gave the best result, which is what it looked like when we looked at the PCA graphs. We also saw that our scaling (either normalization or z-score) yielded better results than the automatic scaling of the function. We saved the SVM results of our normalized data with a linear kernel to compare with the rest of the algorithms.
```{r svm save best, message=FALSE, warning=FALSE}
best_stats<-svm_best_stats["linear norm",] %>% 
  select( method,Accuracy,Sensitivity,Precision,Specificity)
 #best_stats<- svm_best_stats["linear norm",]
...
pred_knn_3_z1 <- knn(train = train_set_z_new[,-21], test = test_set_z_new[,-21], cl = data_train[,63], k=9)
knn_table_3_z1<-CrossTable(x = data_test[,63], y = pred_knn_3_z1, prop.chisq=FALSE)[["t"]]
best_knn_stats["manually chosen K z new",] <- data.frame(Accuracy=accuracy(knn_table_3_z1)$.estimate,
                           Sensitivity=round(sensitivity(knn_table_3_z1),9), 
                           Precision=round(precision(knn_table_3_z1),9), 
                           Specificity=round(specificity(knn_table_3_z1),9),k="k=9")
```
...
```
```{r message=FALSE, warning=FALSE, echo=FALSE,results='hide'}
train_with_knn_repeatedcv_z1<-train(train_set_z_new[,-21],data_train[,63],
                       method = "knn",
                       tuneGrid = data.frame(k=seq(1,30,2)),
                       trControl = trainControl(method = "cv", number = 10, repeats = 10),
                       metric = "Kappa")

knn_predictions_rep_cv_z1<-predict(train_with_knn_repeatedcv_z1, test_set_z_new[,-21])
knn_repcv_mat_z1 <- confusionMatrix(knn_predictions_rep_cv_z1,data_test[,63])

knn_z_table1<-CrossTable(x = data_test[,63], y = knn_predictions_rep_cv_z1, prop.chisq=FALSE)
df_z1<-data.frame(predictions=c(as.numeric(knn_predictions_rep_cv_z1)),labels = c(data_test[,63]))
best_knn_stats["cv z score new",] <- data.frame(
                          Accuracy=knn_repcv_mat_z1[["overall"]][["Accuracy"]],
                           Sensitivity=knn_repcv_mat_z1[["byClass"]][["Sensitivity"]], 
                           Precision=round(knn_repcv_mat_z1[["byClass"]][["Precision"]],3), 
                           Specificity=round(knn_repcv_mat_z1[["byClass"]][["Specificity"]],3),
                           K=paste0("k=", train_with_knn_repeatedcv_z1[["bestTune"]][["k"]])) 
```
```{r message=FALSE, warning=FALSE}
...
best_stats["KNN",]<-svm_best_stats["linear norm",] %>% 
  select( method,Accuracy,Sensitivity,Precision,Specificity)
```
...
best_dt_stats <- data.frame(method="default",
                          Accuracy=DT_pred_tbl[["overall"]][["Accuracy"]],
                           Sensitivity=DT_pred_tbl[["byClass"]][["Sensitivity"]], 
...
best_dt_stats[nrow(best_dt_stats)+1,] <- data.frame(method="boosting with 10 trials",
                          Accuracy=DT_boost_pred10_tbl[["overall"]][["Accuracy"]],
                           Sensitivity=DT_boost_pred10_tbl[["byClass"]][["Sensitivity"]], 
...
best_dt_stats[nrow(best_dt_stats)+1,] <- data.frame(method="fine tuning with cost matrix",
                          Accuracy=dt_cost_pred_tbl[["overall"]][["Accuracy"]],
                           Sensitivity=dt_cost_pred_tbl[["byClass"]][["Sensitivity"]], 
...
best_rf_stats <- data.frame(method="RF default",
                          Accuracy=rf_testing_pred_tbl[["overall"]][["Accuracy"]],
                           Sensitivity=rf_testing_pred_tbl[["byClass"]][["Sensitivity"]], 
...
```{r message=FALSE, warning=FALSE}
tree_res %>%  show_best("roc_auc") # best tree
```
