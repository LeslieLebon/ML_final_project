...
Glaucoma is a disease that damages your eye’s optic nerve, and is the most common cause of irreversible blindness. It usually happens when fluid builds up in the front part of your eye. That extra fluid increases the pressure in your eye, damaging the optic nerve. This disease affects people from 40 years old to 60 years old. 
In many cases, glaucoma may be asymptomatic, and so is not treated until it is too late (in some developing countries, 90% of glaucoma is undetected).

In most cases, blindness can be prevented with appropriate control and treatment. A better awareness and a simple consultation once a year in the ophthalmologist could prevent visual disability in many people.
An ophtalomogist can detect any abnormal pressure in eyes and can do some specific measurement with the help of
OCT angiography (Optical Coherence Tomography Angiography). A classification of normal and sick patients can then help us predict if a patient has or could have glaucoma before it is too late.
...
We have data from 196 patients, half sick with glaucoma and half are healthy, with 63 features (from theOCT angiography).
...
We wanted to observe if we can  cluster patients healthy or with glaucoma with all the features.
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8, fig.height=2}
grid.arrange(default_pca_plot +
...
```{r svm_rbf, message=FALSE, warning=FALSE, results='hide'}
...
```
```{r svm_poly, message=FALSE, warning=FALSE, results='hide'}
Class_classifier_poly <-  ksvm(x=Class ~., data=data_train,kernel = "polydot")
```
```{r SVM Polynomial, message=FALSE, warning=FALSE, echo=FALSE, results='hide'}
...
knitr::kable(svm_stats, caption = 'SVM results- default')
...
```{r message=FALSE, warning=FALSE, echo=FALSE, fig.height=3, fig.width=7, results='hide'}
...
```
```{r manual k choice, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.show='hide'}
...
knitr::kable(knn_seperate_cv_stats, caption = 'KNN results- manually chosen')
...
```{r KNN cross validation stats, message=FALSE, warning=FALSE, echo=FALSE, fig.show='hide', results='hide'}
...
knitr::kable(knn_cv_stats, caption = 'KNN results- CV')
``` 
```{r KNN cross validation stats new, message=FALSE, warning=FALSE, echo=FALSE, fig.show='hide', results='hide'}
...
knitr::kable(knn_cv_new_stats, caption = 'KNN results- CV selected')
```
```{r}
print(paste("CV: the whole dataset's score = ", knn_cv_stats$General, "> the abridged dataset's score = ", knn_cv_new_stats$General))
...
knitr::kable(knn_repeatedcv_stats, caption = 'KNN results- repeated CV')
``` 
...
```{r pred_knn_3_z, message=FALSE, warning=FALSE, results='hide', fig.show='hide'}
...
knitr::kable(knn_seperate_cv_stats_z, caption = 'KNN results- manually scaled')
...
knitr::kable(knn_seperate_cv_stats_z_new, caption = 'KNN results- manually scaled selected')
```
```{r}
...
knn_train_repeatedcv_z<-train(train_set_z[,-63],data_train[,63],
                       ...
                       metric = "Kappa")
```
```{r knn z score, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.show='hide'}
...
knitr::kable(knn_cv_stats_z, caption = 'KNN results- CV scaled selected')
...
knitr::kable(knn_cv_stats_z_new, caption = 'KNN results- CV scaled selected')
...
```{r message=FALSE, warning=FALSE, echo=FALSE, fig.height=1}
...
Next we wantd to see if we can improve our model. We did this by changing some things about it. 
...
We these are very good results. Almost too good, but it is on the set we trained on.
...
```{r  Exploring results final_tree, message=FALSE, warning=FALSE,results='hide', fig.show='hide', echo=FALSE}
