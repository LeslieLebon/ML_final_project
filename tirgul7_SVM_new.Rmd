...
glaucoma <- data_frame(read.csv("GlaucomaM.csv"))# read data
...
We want to classify our data according to the' ‘Class’ column, to be able to look at a person data and according to that predict with a certain probability whether or not he might get sick with glaucoma.
...
```{r, message=FALSE, warning=FALSE}
length_of_test_set <-round(0.2*(nrow(glaucoma)),0)
test_set <- glaucoma[sample(length_of_test_set),]
train_set <- anti_join(glaucoma, test_set)
train_set_Classs <-train_set["Class"]
test_set_Classs <-test_set["Class"]
...
And finally, we are ready to run our algorithm. We'll start with K of 3, the the number of Classs we have and also an odd number, reducing the change of a tie vote.
```{r, message=FALSE, warning=FALSE,results='hide'}
pred_knn <- knn(train = train_set, test = test_set, cl = train_set_Classs, k=21)
knn_table<-CrossTable(x = test_set_Classs, y = pred_knn, prop.chisq=FALSE)
acc_knn3 <- sum(diag(knn_table[["t"]])) / sum(knn_table[["t"]])
df<-data.frame(predictions=c(as.numeric(pred_knn)),labels = c(test_set_Classs))
...
```{r, message=FALSE, warning=FALSE,results='hide'}
pred_knn_norm <- knn(train = train_set_norm, test = test_set_norm, cl = train_set_Classs, k=3)
knn_norm_table<-CrossTable(x = test_set_Classs, y = pred_knn_norm, prop.chisq=FALSE)
acc_knn3_norm <- sum(diag(knn_norm_table[["t"]])) / sum(knn_norm_table[["t"]])
df_norm<-data.frame(predictions=c(as.numeric(pred_knn_norm)),labels = c(test_set_Classs))
...
on the other hand- Classs two and three which had no false positives (all seeds that were predicted as two or three were actually two or 3 respectively) and only a few false negatives (14/15 Class 2 seeds (93.3%) and 6/8 Class 3 seeds (75%) were identified correctly). 
Let's check z-score
...
pred_z <- knn(train = train_set_z, test = test_set_z, cl = train_set_Classs, k=3)
knn_z_table<-CrossTable(x = test_set_Classs, y = pred_z, prop.chisq=FALSE)
acc_knn3_z <- sum(diag(knn_z_table[["t"]])) / sum(knn_z_table[["t"]])
df_z<-data.frame(predictions=c(as.numeric(pred_z)),labels = c(test_set_Classs))
...
knn_z_table[["t"]]
```
We have the same decreased accuracy of 92.5% as with thhe normalized, but the AUC plummeted from 0.967 to 0.853. We can also see that with the normalization the 2 and 3 Classs had 100% TP and Class_1 had 100% TN, whereas with the z-score only Class_2 has 100% TP and Class_3 100% TN, whereas Class_1 now has 2/15 (11.8%) FN and 1/16 (6.7%) FP, so even though in both cases we had only 3 mistakes, in the normalization we had a problem just with the over-predictions of 1 while the rest was perfect, but with the z-score we have more problems.. 

### different Ks
We tried with different K's (3, 9 and 15)

```{r echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
pred_k9 <- knn(train = train_set, test = test_set, cl = train_set_Classs, k=9)
knn9_table<-CrossTable(x = test_set_Classs, y = pred_k9, prop.chisq=FALSE)
acc_knn9 <- sum(diag(knn9_table[["t"]])) / sum(knn9_table[["t"]])
df_knn9<-data.frame(predictions=c(as.numeric(pred_k9)),labels = c(test_set_Classs))
...
pred_k15 <- knn(train = train_set, test = test_set, cl = train_set_Classs, k=15)
knn15_table<-CrossTable(x = test_set_Classs, y = pred_k15, prop.chisq=FALSE)
acc_knn15 <- sum(diag(knn15_table[["t"]])) / sum(knn15_table[["t"]])
df_knn15<-data.frame(predictions=c(as.numeric(pred_k15)),labels = c(test_set_Classs))
...
pred_k21 <- knn(train = train_set, test = test_set, cl = train_set_Classs, k=21)
knn21_table<-CrossTable(x = test_set_Classs, y = pred_k21, prop.chisq=FALSE)
acc_knn21 <- sum(diag(knn21_table[["t"]])) / sum(knn21_table[["t"]])
df_knn21<-data.frame(predictions=c(as.numeric(pred_k21)),labels = c(test_set_Classs))
...
```{r, message=FALSE, warning=FALSE, results='hide'}
feat_selec<- knn(train = train_set[-4], test = test_set[-4], cl = train_set_Classs, k=15)
knn9_no4_table<-CrossTable(x = test_set_Classs, y = feat_selec, prop.chisq=FALSE)
...
feat_sel_pred <- knn(train = train_set_z, test = test_set_z, cl = train_set_Classs, k=15)
knn9_no3_table<-CrossTable(x = test_set_Classs, y = feat_sel_pred, prop.chisq=FALSE)
...
feat_sel_pred <- knn(train = train_set_z, test = test_set_z, cl = train_set_Classs, k=21)
knn9_no3_4_table<-CrossTable(x = test_set_Classs, y = feat_sel_pred, prop.chisq=FALSE)
