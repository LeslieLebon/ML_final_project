...
```{r message=TRUE, warning=TRUE}
svm_linear_stats <- data.frame(kernel="linear",precision=precision(svm_linear_tbl)$.estimate, recall= recall(svm_linear_tbl)$.estimate, accuracy=accuracy(svm_linear_tbl)$.estimate)
print(svm_linear_stats)
# calculate true/false values
agreement_linear <- Class_predictions_linear == test_set$Class
...
svm_rbf_stats <- data.frame(kernel="linear",precision=precision(svm_rbf_tbl)$.estimate, recall= recall(svm_rbf_tbl)$.estimate, accuracy=accuracy(svm_rbf_tbl)$.estimate)
print(svm_rbf_stats)
...
```{r message=TRUE, warning=TRUE}
# compare performance
roc.test(pROC_svm_linear,pROC_svm_rbf)
g2 <- ggroc(list(vanilladot=pROC_svm_linear, rbf=pROC_svm_rbf))+labs(title = "vanilladot Vs. RBF")
g2+theme(legend.position = "top")+ labs(color="kernel type")
```
```{r message=TRUE, warning=TRUE}

# with additional aesthetics:
g<- ggroc(pROC_svm_rbf)

# And change axis labels to FPR/FPR
gl <- ggroc(pROC_svm_rbf, legacy.axes = TRUE)+ ggtitle("rbf ROC curve") 
...
g.list + scale_colour_manual(values = c("red", "blue", "black"))+ ggtitle("2")
...
# OR faceting
g.list + facet_grid(.~Class) + theme(legend.position="none")+ ggtitle("6")
