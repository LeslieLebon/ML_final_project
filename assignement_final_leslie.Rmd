...
```{r, message=FALSE, warning=FALSE, results='hide'}
glaucoma$Class<-as.factor(glaucoma$Class)
glaucoma$Class <- factor(glaucoma$Class, levels = c("glaucoma", "normal"), labels = c("g", "n"))
```
...
```{r, message=FALSE, warning=FALSE,results='hide'}
#replace glaucoma class by 1 and normal class by 0

test_set_labels<-train_set$Class
train_set2<-train_set[,-63]
pred_knn <- knn.cv(train = train_set2, cl =test_set_labels , k=21)
# I got an error:
...
train_set_norm <- as.data.frame(lapply(train_set[,-63], normalize))
test_set_norm <- as.data.frame(lapply(test_set[,-63], normalize))
...
test_set_labels<-train_set_norm$Class
train_set_norm2<-train_set_norm[,-63]
pred_knn_norm <- knn(train = train_set_norm2, test = test_set_norm, cl = train_set_labels, k=3)
#i got an error:
