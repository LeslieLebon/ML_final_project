...
```{r message=FALSE, warning=FALSE}
best_knn_stats["manually, scaled, selected",] = knn_seperate_cv_stats_z_new
```
...
Interestingly, the best results were with the manually chosen K's (when K=3). using CV and repeated CV the best resukt was with K=17. We had manually chosen K=15 and K=21, and both were not as good as K=3.
...
#print.data.frame(best_knn_stats %>% select(Accuracy,Sensitivity, Precision, Specificity,General))
...
```
```{r message=FALSE, warning=FALSE,echo=FALSE}
knitr::kable(best_dt_stats , caption = ' best DT results')%>%
  kable_paper("striped", full_width = F)
```
...
```
```{r message=FALSE, warning=FALSE,echo=FALSE}
knitr::kable(best_dt_stats , caption = ' best DT after boost results')%>%
  kable_paper("striped", full_width = F) 
```
...
#print(error_cost)
knitr::kable(error_cost , caption = ' error cost')%>%
  kable_paper("striped", full_width = F) 
```
...
knitr::kable(dt_cost_pred_tbl[["table"]] , caption = ' confusion matrix ')%>%
  kable_paper("striped", full_width = F) 
```
#DT conclusion
...
With the boosted model, we have 72% of chances that the model distinguish sick patients to healthy patients.
 Regarding the different results obtained the default DT presents best predictions than boosted and with cost matrix with a general score of 87.4% compared to boosted with 83.3% or with cost matrix : 75%.

```{r message=FALSE, warning=FALSE, fig.height=3, fig.cap="Desition Tree", fig.width=6,echo=FALSE}
#plot(dt_cost)
```
...
knitr::kable(best_rf_stats , caption = ' best RF results')%>%
  kable_paper("striped", full_width = F)
...
knitr::kable(tree_res %>%  show_best("roc_auc"), caption = 'best tree')
...
```{r  Exploring results final_tree, message=FALSE, warning=FALSE, echo=FALSE}
