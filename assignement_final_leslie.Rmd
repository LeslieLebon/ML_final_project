...
Our objective in this project was to analyze the “GlaucomaM” dataset (where the 'Class' of the seed is the column used for labeling and classifying) in order to be able to look at a person's data and according to that predict with a certain probability whether or not he might get sick with glaucoma.
...
Class_classifier_vanilladot <-  ksvm(x=Class~., data=train_set,kernel = "vanilladot")
Class_classifier_vanilladot
```
...
Class_predictions_vanilladot <- predict(Class_classifier_vanilladot, test_set)
head(Class_predictions)
table(Class_predictions_vanilladot, test_set$Class)
```

To examine how well our classifier performed, we need to compare the predicted
Class to the true Class in the testing dataset, so we used the table() function.
...
agreement_vanilladot <- Class_predictions_vanilladot == test_set$Class
# true/false preformance percentage
prop.table(table(agreement_vanilladot))
df_vanilladot<-data.frame(predictions=c(as.numeric(Class_predictions_vanilladot)),labels = c(test_set$Class))
   
pROC_svm_vanilladot <- roc(df_vanilladot$labels,df_vanilladot$predictions,
            smoothed = TRUE,
            ci=TRUE, ci.alpha=0.9, stratified=FALSE,
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
            print.auc=TRUE, show.thres=TRUE)
sens.ci <- ci.se(pROC_svm_vanilladot)
...
```{r}
# predict on test set
Class_predictions_rbf <- predict(Class_classifier_rbf, test_set)
```

And compare it to the previous model:
```{r}
# calculate performance
agreement_rbf <- Class_predictions_rbf == test_set$Class
prop.table(table(agreement_rbf))
df_rbf<-data.frame(predictions=c(as.numeric(Class_predictions_rbf)),labels = c(test_set$Class))
pROC_svm_rbf <- roc(df$labels,df$predictions,
            smoothed = TRUE,
            ci=TRUE, ci.alpha=0.9, stratified=FALSE,
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
            print.auc=TRUE, show.thres=TRUE)
...
```{r}
# compare performance
roc.test(pROC_svm,pROC_svm_rbf)
g2 <- ggroc(list(vanilladot=pROC_svm_vanilladot, rbf=pROC_svm_rbf))
