...
df<-data.frame(predictions=c(as.numeric(Class_predictions)),labels = c(test_set$Class))
...
df<-data.frame(predictions=c(as.numeric(Class_predictions_rbf)),labels = c(test_set$Class))
pROC_svm_imprvd <- roc(df$labels,df$predictions,
            smoothed = TRUE,
            ci=TRUE, ci.alpha=0.9, stratified=FALSE,
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
            print.auc=TRUE, show.thres=TRUE)
sens.ci <- ci.se(pROC_svm_imprvd)
plot(sens.ci, Class="shape", col="lightblue")
plot(sens.ci, Class="bars")
```
