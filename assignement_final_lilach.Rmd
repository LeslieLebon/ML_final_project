...
are numeric, so there is no need to work on that.
...
can observe 8 clusters of features. Results obtained with heatmap: we
can observe one big cluster that could be divided in 2 and 2 others
clusters of features.

> > > > > > > > > > > > > > > > > > > > It means that ??? we should
> > > > > > > > > > > > > > > > > > > > delete features that are
> > > > > > > > > > > > > > > > > > > > correlated?????? what do you
> > > > > > > > > > > > > > > > > > > > think Lilach?
...
In the PCA, we can observe that most of glaucoma patients and normal
patients are clustered in 2 groups, but some of them are mixed, probably
to the fact that many features correlate.

Let's try to observe the different pvalues of correlation, to determine
which feature are correlated.
...
data<-train_set
data_new_85 <- data[ , !apply(cor_matrix_rm, 2, function(x) any(x > 0.85))]
data_new_80 <- data[ , !apply(cor_matrix_rm, 2, function(x) any(x > 0.8))]
data_new_75 <- data[ , !apply(cor_matrix_rm, 2, function(x) any(x > 0.75))]

# Preform PCA on our new data
gl_pca_85 <- prcomp(data_new_85)
gl_pca_80 <- prcomp(data_new_80)
gl_pca_75 <- prcomp(data_new_75)
...
```{r, message=FALSE, warning=FALSE}
gl_pca_plot_85 <- autoplot(gl_pca_85,
                          data = glaucoma,
                          colour = 'Class')
gl_pca_plot_80 <- autoplot(gl_pca_80,
                          data = glaucoma,
                          colour = 'Class')
gl_pca_plot_75 <- autoplot(gl_pca_75,
                          data = glaucoma,
                          colour = 'Class')
gl_pca_plot
gl_pca_plot_85
gl_pca_plot_80
gl_pca_plot_75
grid.arrange(gl_pca_plot,gl_pca_plot_85,gl_pca_plot_80,gl_pca_plot_75, ncol=2,nrow=2)
...
When the data is not linearly separable, we can use a support vector classifier (SVC)
(sub-method of SVM) or even SVM. 

## MMC
In cases where the data is linearly separable (can be easily separated we can simply use a Maximal Margin Classifier (MMC) to separate the data. We will use the data as we procured it from the PCA and dimentionality reduction.
In the PCA, we saw that most of glaucoma patients and normal
patients are clustered in 2 groups, but some of them are mixed, probably due to the fact that many features correlate.
```{r}
# Construct sample data set - we'll randomly choose 2 features for the axes
y2=train_set[63]
y=train_set[,63]
train_pca <- prcomp(train_set[,1:62],
                   center = TRUE,
                   scale. = TRUE)
dat_train <- data.frame(x=as.matrix(train_pca[["x"]]), y=as.factor(train_set[,63]))
dat_train_85 <- data.frame(x=as.matrix(gl_pca_85[["x"]]), y=as.factor(train_set[,63]))
dat_train_80 <- data.frame(x=as.matrix(gl_pca_80[["x"]]), y=as.factor(train_set[,63]))
dat_train_75 <- data.frame(x=as.matrix(gl_pca_75[["x"]]), y=as.factor(train_set[,63]))


# Plot data
gg_train<-ggplot(data = dat_train, aes(x = x.PC1, y = x.PC2, color = y, shape = y)) + 
  geom_point(size = 2) +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  theme(legend.position = "none")
gg_train_85<-ggplot(data = dat_train_85, aes(x = x.PC1, y = x.PC2, color = y, shape = y)) + 
  geom_point(size = 2) +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  theme(legend.position = "none")
gg_train_80<-ggplot(data = dat_train_80, aes(x = x.PC1, y = x.PC2, color = y, shape = y)) + 
  geom_point(size = 2) +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  theme(legend.position = "none")
gg_train_75<-ggplot(data = dat_train_75, aes(x = x.PC1, y = x.PC2, color = y, shape = y)) + 
  geom_point(size = 2) +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  theme(legend.position = "none")

grid.arrange(gg_train,gg_train_85,gg_train_80,gg_train_75, ncol=2,nrow=2)

```

```{r}
# Construct sample data set - we'll randomly choose 2 features for the axes
train_pca <- prcomp(glaucoma[,1:62],
                   center = TRUE,
                   scale. = TRUE)
...
train_pca_plot <- autoplot(train_pca,
                          data = glaucoma,
                          colour = 'Class')
x<-as.matrix(train_pca_plot[["data"]][1:65])
y <- x[,"Class"]

dat_train <- data.frame(x=x[,1:64], y=as.factor(y))

# Plot data
ggplot(data = dat_train[,1:2], aes(x = x.PC1, y = x.PC2, color = y, shape = y)) + 
  geom_point(size = 2) +
  scale_color_manual(values=c("#000000", "#FF0000")) +
  theme(legend.position = "none")
```
...
svmfit <- svm(Class~., data = train_set, kernel = "linear", scale = FALSE)
# Plot Results
...
kernfit <- ksvm(x=y~., data=dat_train, Class = "C-svc", kernel = 'vanilladot')
#plot(kernfit, data = dat_train)
Class_predictions_linear <- predict(kernfit, test_set)
...
ypred <- predict(bestmod, dat_train)
(misclass <- table(predict = ypred, truth = dat_train$y))
grid.arrange(tableGrob(misclass),ncol=1,nrow=1)
...
classes against true classes using the predict() .
...
svm_linear_stats <- data.frame(method="linear_kernel",precision=precision(svm_linear_tbl)$.estimate, recall= recall(svm_linear_tbl)$.estimate, accuracy=accuracy(svm_linear_tbl)$.estimate, AUC=pROC_svm_linear$au)
...
```{r}
# find optimal cost of mis-classification
tune.out <- tune(svm, Class~., data = train_set, kernel = "linear",
                 ranges = list(cost = c(0.001, 0.01, 0.1, 1, 5, 10, 100)))
# extract the best model
bestmod <- tune.out$best.model
ypred <- predict(bestmod, test_set)
misclass <- table(predict = ypred, truth = test_set$Class)
grid.arrange(tableGrob(misclass),ncol=1,nrow=1)

df_bestmod<-data.frame(predictions=c(as.numeric(ypred)),labels = c(test_set$Class))
pROC_svm_linear_bestmod <- roc(df_bestmod$labels,df_bestmod$predictions,
            smoothed = TRUE,
            ci=TRUE, ci.alpha=0.9, stratified=FALSE,
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
            print.auc=TRUE, show.thres=TRUE)

nrow=nrow(svm_linear_stats)+1
svm_linear_stats <- data.frame(method=matrix(c(svm_linear_stats$method,"bestmod with tune.out"),nrow=nrow),
                               precision=matrix(c(svm_linear_stats$precision, precision(misclass)$.estimate),nrow=nrow),
                               recall=matrix(c(svm_linear_stats$recall, recall(misclass)$.estimate),nrow=nrow),
                               accuracy=matrix(c(svm_linear_stats$accuracy, accuracy(misclass)$.estimate),nrow=nrow),
                        AUC=matrix(c(svm_linear_stats$AUC,pROC_svm_linear_bestmod$au),nrow=nrow))
...
nrow=nrow(svm_linear_stats)+1
svm_linear_stats <- data.frame(method=matrix(c(svm_linear_stats$method,"rbf"),nrow=nrow),
                               precision=matrix(c(svm_linear_stats$precision, precision(svm_rbf_tbl)$.estimate),nrow=nrow),
                               recall=matrix(c(svm_linear_stats$recall, recall(svm_rbf_tbl)$.estimate),nrow=nrow),
                               accuracy=matrix(c(svm_linear_stats$accuracy, accuracy(svm_rbf_tbl)$.estimate),nrow=nrow), 
                          AUC=matrix(c(svm_linear_stats$AUC,pROC_svm_rbf$au),nrow=nrow))
...
```{r}
# find optimal cost of mis-classification
tune.out <- tune(svm, Class~., data = train_set, kernel = "radial",
                 ranges = list(cost = c(0.001, 0.01, 0.1, 1, 5, 10, 100)))
# extract the best model
bestmod <- tune.out$best.model
ypred <- predict(bestmod, test_set)
...
nrow=nrow(svm_linear_stats)+1
svm_linear_stats <- data.frame(method=matrix(c(svm_linear_stats$method,"tune.out with rbfdot"),nrow=nrow),
                               precision=matrix(c(svm_linear_stats$precision, precision(misclass)$.estimate),nrow=nrow),
                               recall=matrix(c(svm_linear_stats$recall, recall(misclass)$.estimate),nrow=nrow),
                               accuracy=matrix(c(svm_linear_stats$accuracy, accuracy(misclass)$.estimate),nrow=nrow),
                        AUC=matrix(c(svm_linear_stats$AUC,pROC_svm_linear_bestmod$au),nrow=nrow))
